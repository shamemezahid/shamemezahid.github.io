    ...
    <div className="text-sm w-full flex flex-col justify-start gap-2 p-4 rounded-xl">
      ...
      <div className="w-full grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-3 items-start -mx-4 transition-all duration-200">
        {links.map((link, index) => (
          <a
            key={index}
            className="group flex items-center h-12 font-semibold text-emerald-700 dark:text-emerald-500 transition-all duration-500 px-4 py-3 rounded-full hover:bg-emerald-50 dark:hover:bg-emerald-950"
            target="_blank"
            href={link.url}
