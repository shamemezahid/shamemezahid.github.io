    ...
    <div className="text-sm w-full flex flex-col justify-start gap-2 p-4 rounded-xl" aria-label="Social Links of Shamim Bin Zahid">
      ...
      <div className="w-full grid grid-cols-2 sm:flex sm:flex-wrap gap-1 sm:gap-2 items-start -mx-4 transition-all duration-200">
        {links.map((link, index) => (
          <a
            key={index}
            className="hover:shadow-primary-700/25 hover:shadow-2xl group flex items-center h-12 font-semibold text-primary-700 dark:text-primary-500 transition-all duration-500 px-4 py-3 rounded-2xl hover:bg-primary-50 dark:hover:bg-primary-950"
            target="_blank"
            href={link.url}
